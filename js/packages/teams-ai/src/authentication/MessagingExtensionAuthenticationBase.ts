import { ActivityTypes, InvokeResponse, TokenExchangeRequest, TokenResponse, TurnContext } from 'botbuilder';
...
export abstract class MessagingExtensionAuthenticationBase {
    public async authenticate(context: TurnContext, settings: any): Promise<string | undefined> {
        const value = context.activity.value;
        const tokenExchangeRequest = value.authentication;

        // Reveived SSO token, this happens when a silentAuth action is sent to Teams
        if (tokenExchangeRequest && tokenExchangeRequest.token) {
            // Message extension token exchange invoke activity
            try {
                const tokenExchangeResponse = await this.handleSsoTokenExchange(context, tokenExchangeRequest);
